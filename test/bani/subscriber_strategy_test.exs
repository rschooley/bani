defmodule Bani.SubscriberStrategyTest do
  ...
  describe "perform/5 :exactly_once" do
    ...
    test "lock_subscriber fails" do
      ...
      stub(Bani.MockSubscriberStore, :lock_subscriber, fn (_, _) ->
        Process.send(test_pid, {:expect_lock_subscriber_called, ref_lock}, [])
      ...
      assert capture_log(fn ->
        assert {:error, %Bani.SubscriberDeliverError{reason: :store_lock_failed}} = Bani.SubscriberStrategy.perform(@strategy, tenant, subscriber_key, Bani.MockSubscriberStore, process_fn)
      end) =~ "some error"
    ...
    test "process_fn partially fails, update store succeeds" do
      test_pid = self()
      ref_lock = make_ref()
      ref_process = make_ref()
      ref_unlock = make_ref()
      ...
      process_fn = fn _ ->
        ...
        {:partial_error, "some error", new_acc, inc_count}
      end
      ...
      assert capture_log(fn ->
        assert {:error, %Bani.SubscriberDeliverError{reason: :handler_partial_error}} = Bani.SubscriberStrategy.perform(@strategy, tenant, subscriber_key, Bani.MockSubscriberStore, process_fn)
      end) =~ "some error"
    ...
    test "process_fn partially fails, update store fails" do
      test_pid = self()
      ref_lock = make_ref()
      ref_process = make_ref()
      ref_unlock = make_ref()
      ...
      assert capture_log(fn ->
        assert {:error, %Bani.SubscriberDeliverError{reason: :store_update_failed_after_partial_handler_failed}} = Bani.SubscriberStrategy.perform(@strategy, tenant, subscriber_key, Bani.MockSubscriberStore, process_fn)
      end) =~ "some update error"
    ...
    test "unlock_subscriber fails" do
      ...
      assert capture_log(fn ->
        assert {:error, %Bani.SubscriberDeliverError{reason: :store_unlock_failed}} = Bani.SubscriberStrategy.perform(@strategy, tenant, subscriber_key, Bani.MockSubscriberStore, process_fn)
      end) =~ "some error"
